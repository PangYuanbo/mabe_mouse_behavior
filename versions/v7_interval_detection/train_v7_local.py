"""
V7 Local Training - Using Cached Windows (Fast startup with preprocessed data)
Run from project root: python train_v7_local.py

Prerequisites:
- Run preprocess_windows.py first to generate cached windows
"""
...
from src.utils.dual_metric_trainer import DualMetricTrainer
...
def main():
    ...
    # Create dataloaders (using real-time window generation)
    print(f"\nLoading Kaggle data with motion features...")
    print(f"  Batch size: {config.get('batch_size', 768)}")
    ...
    train_loader, val_loader = create_kaggle_dataloaders(
        ...
        batch_size=config.get('batch_size', 768),
    ...
    # Create trainer with Kaggle competition metrics
    print("="*60)
    print("Initializing trainer with competition metrics...")
    print("="*60)

    trainer = DualMetricTrainer(
        model=model,
    ...
    print(f"  Best Frame F1: {trainer.best_frame_f1:.4f}")
    print(f"  Best Interval F1 (Kaggle): {trainer.best_interval_f1:.4f}")
