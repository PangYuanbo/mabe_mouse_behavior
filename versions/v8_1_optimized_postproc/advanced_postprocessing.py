"""
V8.1 Optimized Post-processing for Interval Detection
Improvements over V8:
1. Motion-gated filtering (velocity thresholds for escape/chase/freeze)
2. Fine-tuned class-specific thresholds
3. Optimized boundary refinement
"""
...
# V8.1: Fine-tuned class-specific configurations
CLASS_CONFIG = {
    # Sniffing behaviors (balanced precision/recall)
    'sniff': {
        'min_duration': 6,
        'prob_threshold': 0.38,
        'merge_gap': 5,
        'max_required': 0.50  # V8.1: raised to control FP
    },
    'sniffgenital': {
        'min_duration': 3,
        'prob_threshold': 0.26,  # V8.1: lowered for recall
        'merge_gap': 5,
        'frac_high_threshold': 0.20
    },
    'sniffface': {
        'min_duration': 3,
        'prob_threshold': 0.28,  # V8.1: kept
        'merge_gap': 5,
        'frac_high_threshold': 0.20
    },
    'sniffbody': {
        'min_duration': 4,
        'prob_threshold': 0.28,  # V8.1: lowered slightly
        'merge_gap': 6,  # V8.1: increased for easier merging
        'frac_high_threshold': 0.20
    },
    'reciprocalsniff': {
        'min_duration': 4,
    ...
    # Mating behaviors (precision-focused)
    'mount': {
        'min_duration': 4,
        'prob_threshold': 0.43,  # V8.1: slightly raised
        'merge_gap': 8
    },
    'intromit': {
        'min_duration': 3,
        'prob_threshold': 0.45,  # V8.1: raised for precision
        'merge_gap': 10
    },
    'attemptmount': {
        'min_duration': 3,
    ...
    # Aggressive behaviors with motion gating
    'attack': {
        'min_duration': 4,
        'prob_threshold': 0.40,
        'merge_gap': 5
    ...
    'chase': {
        ...
        'prob_threshold': 0.40,
        'merge_gap': 5,
        'velocity_min': 80.0  # V8.1: px/s, high-speed requirement
    },
    'chaseattack': {
        'min_duration': 4,
        'prob_threshold': 0.40,
    ...
    'approach': {
        'min_duration': 4,
        'prob_threshold': 0.42,  # V8.1: raised to control noise
        'merge_gap': 4
    },
    'follow': {
        'min_duration': 6,
    ...
    'escape': {
        'min_duration': 5,
        'prob_threshold': 0.55,  # V8.1: kept high
        'merge_gap': 4,
        'max_required': 0.65,
        'velocity_min': 80.0  # V8.1: px/s, high-speed requirement (critical for FP reduction)
    },
    'shepherd': {
        'min_duration': 5,
        'prob_threshold': 0.35,
...
def motion_gating_filter(
    ...
    Args:
        ...
        keypoints: [T, 56] original keypoint coordinates (before motion features)
        action_names: Mapping from action_id to action name
        action_probs: [T, num_actions] probabilities (for freeze threshold relaxation)
        fps: Video frame rate

    Returns:
        filtered_intervals: Intervals passing motion gating
    """
    filtered = []
    ...
    for interval in intervals:
        ...
        # Check if this action has velocity requirements
...
def probs_to_intervals_advanced(
    ...
    # 6) V8.1: Motion gating filter (escape/chase/freeze)
    if keypoints is not None:
        intervals = motion_gating_filter(intervals, keypoints, action_names, action_probs)
