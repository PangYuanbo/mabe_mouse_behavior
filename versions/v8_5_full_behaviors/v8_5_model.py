"""
V8.5 Multi-task Behavior Detection Model
Predicts: Action (38 classes) + Agent (4 mice) + Target (4 mice)

Key changes from V8/V8.1:
- NUM_ACTIONS = 38 (was 28) - includes all 37 behaviors + background
- Same architecture as V8, just different output dimension
"""
...
class V85BehaviorDetector(nn.Module):
    """
    Multi-task model for fine-grained behavior detection (V8.5)

    Outputs:
        action_logits: [B, T, 38] - behavior classification (all 37 behaviors)
    ...
    def __init__(
        ...
        # V8.5: Output 38 classes (was 28 in V8)
        self.action_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(256, num_actions)  # 38 classes in V8.5
        )
    ...
    def forward(self, x):
        ...
        Returns:
            action_logits: [B, T, num_actions] (38 in V8.5)
...
class V85MultiTaskLoss(nn.Module):
    """
    Combined loss for multi-task learning (V8.5)
    Same as V8, but with 38 action classes
    """
    ...
    def __init__(
        ...
        use_focal=True,
        focal_gamma=2.0,
        class_weights=None  # V8.5: Should be [38] tensor if provided
    ):
        super().__init__()
