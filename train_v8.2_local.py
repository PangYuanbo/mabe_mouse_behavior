...
def validate(model, val_loader, criterion, device, compute_interval_f1=False, use_advanced_postproc=False):
    ...
    if compute_interval_f1:
        ...
        if use_advanced_postproc:
            ...
            # V8.2: Temporal smoothing with kernel_size=3 (better for short segments)
            action_probs_smooth = temporal_smoothing(action_probs_flat, kernel_size=3, method='median')
            agent_probs_smooth = temporal_smoothing(agent_probs_flat, kernel_size=3, method='median')
            target_probs_smooth = temporal_smoothing(target_probs_flat, kernel_size=3, method='median')
...
def main():
    parser = argparse.ArgumentParser(description='V8.2 Fine-Tuned Multi-task Training')
    ...
    print("="*60)
    print("V8.2 Fine-Tuned Multi-task Behavior Detection")
    print("Smoothing=3 + Lower Sniff Thresholds + Motion Gating")
    print("="*60)
    ...
    best_val_acc = 0
    best_kaggle_f1 = 0  # Track best Kaggle F1 score
    patience_counter = 0
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        # Validate (compute Kaggle interval F1 EVERY epoch for V8.1)
        compute_f1 = True  # V8.1: Always compute Kaggle metrics
        ...
        # V8.1: Save best model based on Kaggle F1 score
        if 'interval_f1' in val_metrics:
            ...
            if current_f1 > best_kaggle_f1:
                ...
                patience_counter = 0  # Reset patience on Kaggle F1 improvement
            else:
                patience_counter += 1
        else:
            # Fallback if F1 not computed (shouldn't happen in V8.1)
            patience_counter += 1
